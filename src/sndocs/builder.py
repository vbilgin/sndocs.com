...
SITE_ENGINE_PACKAGES = ("mkdocs", "mkdocs-material")
...
def pipeline_fingerprint(
    ...
    site_engine_versions: dict[str, str] | None = None,
) -> str:
    ...
    # A site-generator upgrade (MkDocs, Material for MkDocs) can change every
    # generated byte without any pipeline file or configuration changing, so
    # it must invalidate cached families the same way a code change does.
    engines = site_engine_versions or {
        name: importlib.metadata.version(name) for name in SITE_ENGINE_PACKAGES
    }
    for name in SITE_ENGINE_PACKAGES:
        digest.update(f"\0site-engine\0{name}\0".encode())
        digest.update(engines[name].encode())
